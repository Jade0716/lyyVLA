...
@FRAMEWORK_REGISTRY.register("QwenGR00T_ActionToken_DINO_8token")
class Qwen_GR00T_ActionToken_DINO_8token(Qwen_GR00T_ActionToken):
    """
    ...
    Qwen hidden size, compresses them with a small Q-Former resampler, and the
    action head receives:

        [action query hidden states, compressed DINO tokens]
    ...
    def __init__(self, config=None, **kwargs) -> None:
        ...
        self.dino_num_query_tokens = int(dino_cfg.get("num_query_tokens", dino_cfg.get("dino_num_query_tokens", 8)))
        self.dino_query_tokens = nn.Parameter(torch.randn(1, self.dino_num_query_tokens, hidden_size) * 0.02)
        self.dino_qformer = nn.ModuleList(
            [
                CrossAttentionBlock(
                    hidden_dim=hidden_size,
                    num_heads=int(dino_cfg.get("qformer_num_heads", 8)),
                    dropout=float(dino_cfg.get("qformer_dropout", 0.1)),
                )
                for _ in range(int(dino_cfg.get("qformer_num_layers", 2)))
            ]
        )
    ...
    def _encode_dino_hidden_states(
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            query = self.dino_query_tokens.to(device=dino_hidden.device, dtype=dino_hidden.dtype)
            query = query.expand(batch_size, -1, -1)
            for layer in self.dino_qformer:
                query = layer(query, dino_hidden)
