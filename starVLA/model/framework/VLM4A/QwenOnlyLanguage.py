...
@FRAMEWORK_REGISTRY.register("QwenOnlyLanguage")
...
# 
    def __init__(
        ...
        **kwargs,
        ...
        num_vl_layers, llm_hidden_size = self.qwen_vl_interface.model.config.text_config.num_hidden_layers, self.qwen_vl_interface.model.config.hidden_size
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            if input_ids is not None:
                ...
                # For each layer, select only non-vision token hidden states
                vl_embs_list = []
                for layer_hidden in all_hidden[-expected_layers:]:
                    # layer_hidden shape: (B, seq_len, hidden_dim)
                    if layer_hidden.shape[1] == len(non_vision_indices) + vision_token_mask.sum():
                        ...
                        vl_embs_list.append(layer_hidden_filtered)
                    else:
                        # Already filtered (e.g., in some VLM implementations)
                        vl_embs_list.append(layer_hidden)
            else:
